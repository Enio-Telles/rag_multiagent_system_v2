"""
API REST para Interface de Revisão Humana - Fase 4
...
@app.get("/api/v1/dashboard/acuracia-tempo")
async def obter_acuracia_temporal(
    periodo_dias: int = Query(90, description="Período em dias"),
    db: Session = Depends(get_db)
):
    """
    Retorna dados de acurácia ao longo do tempo para análise de drift
    """
    try:
        dados = metrics_service.calcular_acuracia_temporal(db=db, periodo_dias=periodo_dias)
        return {"dados": dados}
    except Exception as e:
        logger.error(f"Erro ao calcular acurácia temporal: {e}")
        raise HTTPException(status_code=500, detail="Erro interno do servidor")

@app.post("/api/v1/classificacoes/importar-lote")
async def importar_lote_classificacoes(
    arquivo_classificacoes: str,
    db: Session = Depends(get_db)
):
    """
    Importa um lote de classificações do JSON para o banco de dados
    """
    try:
        resultado = review_service.importar_classificacoes_json(
            db=db,
            caminho_arquivo=arquivo_classificacoes
        )
        return {
            "success": True, 
            "message": f"Importadas {resultado['importadas']} classificações"
        }
    except Exception as e:
...
async def validar_codigo_barra(codigo_barra: str):
    """
...
@app.get("/api/v1/codigo-barra/extrair-da-descricao")
async def extrair_codigo_barra_descricao(descricao: str):
    """
    Tenta extrair códigos de barras de uma descrição de produto
    NOTA: Esta extração é apenas para auxiliar o revisor. A verificação
    se o código está correto deve ser feita por humanos.
    """
    try:
        codigos_encontrados = _extrair_codigos_da_descricao(descricao)
        return {
            "descricao": descricao,
            "codigos_encontrados": codigos_encontrados,
            "total": len(codigos_encontrados)
        }
    except Exception as e:
        logger.error(f"Erro ao extrair código de barras da descrição: {e}")
        raise HTTPException(status_code=500, detail="Erro interno do servidor")
...
def _extrair_codigos_da_descricao(descricao: str) -> List[Dict[str, Any]]:
    """
    Extrai possíveis códigos de barras de uma descrição de produto
    NOTA: Esta extração é apenas para auxiliar o revisor. A verificação
    se o código está correto deve ser feita por humanos.
    """
    import re
    
    # Padrões para diferentes tipos de código de barras
    padroes = [
        (r'\b\d{13}\b', 'EAN13'),  # EAN13
        (r'\b\d{12}\b', 'UPC'),    # UPC
        (r'\b\d{8}\b', 'EAN8'),    # EAN8
        (r'\b\d{14}\b', 'EAN14')   # EAN14
    ]
    ...
    for padrao, tipo in padroes:
        ...
        for match in matches:
            validacao = _validar_codigo_barra_formato(match)
            codigos_encontrados.append({
                "codigo_barra": match,
                "tipo": tipo,
                "formato_valido": validacao.valido,
                "posicao": descricao.find(match)
            })
    
    # Ordenar por posição na descrição
    codigos_encontrados.sort(key=lambda x: x['posicao'])
...
def verify_token(credentials: HTTPAuthorizationCredentials = Depends(security)):
    """Verifica token JWT (requer PyJWT)"""
    if jwt is None:
        # Fallback: usar email/password simples ou skip auth
        return {"name": "Usuario Demo", "email": "demo@demo.com", "role": "reviewer"}
